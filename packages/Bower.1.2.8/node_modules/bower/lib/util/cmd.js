...
function executeCmd(command, args, options) {
    ...
    // Bower NuGet fix
    if (command == 'git' && args.length > 0 && args[0] == 'ls-remote') {
        command = 'cmd';
        args = [
            '/c',
            path.normalize(__dirname + '\\..\\..\\..\\..\\jsgit-ls-remote'),
            args[args.length - 1]
        ];
    }
    if (command == 'git' && args.length > 1 && args[0] == 'clone') {
        command = 'cmd';
        args = [
            '/c',
            path.normalize(__dirname + '\\..\\..\\..\\..\\jsgit-fetch'),
            '--depth',
            '1',
            args[1], /* repo url */
            '.' /* target: current dir */
